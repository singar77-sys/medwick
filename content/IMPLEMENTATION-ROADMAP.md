...
- Choose rebuild platform (SSR/static: Next.js/Astro), sign off sitemap (the [Site Structure](SITE-STRUCTURE.md)).
...
- **AEO foundations (the top competitive gap, so it leads, not trails).** Spec the schema plan for the rebuild (LocalBusiness, Service, FAQPage per market), draft the first FAQ answer-content blocks written as short, direct, question-first answers useful to homeowners, reserve an optional `/llms.txt` for the rebuild (for AI systems that may consume it; no Google Search effect), and tune the GBP for AI retrieval (categories, services, attributes, Q&A). No incumbent in any market is doing this; being an early, well-structured answer source is an opening no incumbent is contesting.
...
- **Insurance-claim advocacy positioning drafted (the flagship).** Across storm, roofing, and water: a published, customer-first process — you're our customer, not the insurance company's; clear, documented, and honest at every step. Everyone bills insurance; nobody owns the advocacy story, and this is the brand.
...
- **AEO ships with the build, not after.** Optionally publish an experimental `/llms.txt` (for AI systems that may consume it; no Google Search effect); render FAQPage schema and the question-first answer blocks on every hub and emergency page; carry the locked NAP into all page schema. The goal is to be the retrievable answer while every competitor is still AI-invisible.
- **Technical launch checklist (all must pass):** server-rendered content · CWV all "good" on mobile · one phone number sitewide · schema per §4.5 (no `aggregateRating` until reviews render on-page) · FAQ/answer schema valid and machine-readable · self-hosted images w/ descriptive alt · per-section sitemaps submitted · GA4/GSC firing on every page.
- City pages **Wave 1**: Medina, Brunswick, Wadsworth, each passing the 4-element anti-doorway gate.
- Publish cornerstone content 1-3 (the [Content Calendar](CONTENT-CALENDAR.md)).
...
- **Add review markup** once real reviews render → structured data (Google does not show organic stars from self-serving LocalBusiness review markup; stars come from the GBP / map pack).
...
- **AEO leadership consolidated:** own the cited-answer position across roofing and water queries, keep FAQ/answer schema current (and the optional `/llms.txt`, if used), and track AI-answer appearances as a standing KPI. This is the compounding first-mover lane.
...
**GATE 4 / 12-month scorecard:** ≥50 reviews and clear of Fixler's 34, with water on a trajectory toward SERVPRO's 189 ceiling · at least one award landed or in final voting (Reader's Choice) · cited in AI answers for target queries · 40-70 non-brand page-1 keywords · organic a primary lead source · all KPI targets in the [SEO Strategy](SEO-STRATEGY.md), §3 reviewed against actuals.
...
| AEO / schema execution quality | the #1 competitive gap; cited-answer position | Build schema, FAQ answer-content, and NAP consistency into Phase 1-2 (with an optional `/llms.txt`), not as a bolt-on. It fails silently if malformed, so validate machine-readability at GATE 2 |
