...
/insurance-claims/                      Insurance Claims hub  [PRIMARY, build FIRST]
  ...
  /insurance-claims/scope-review/       scope review & repair estimate (educational: options if a claim is denied or underpaid)
...
/llms.txt                               optional/experimental root-level file for AI systems that may consume it (no Google Search effect)
...
- **Must be in server HTML:** the advocacy promise up top (we answer to you, the homeowner), the standard licensed-contractor disclaimer (below), the storm-recovery path (storm damage → insurance claim → roof, water, and rebuild under one company), 5.0★ proof band linked to GBP, reg #26-731, real project photos, service-area list, owner block, an answer-optimized FAQ block (feeds the AI layer and `FAQPage` schema), two CTAs (Get Claim Help / Emergency Call).
...
- **Insurance Claims body:** ≥ 800 words, 100% unique. Leads with the documented, plain-English "clear, documented, and honest at every step" process: we walk you through the claim, help make the calls, meet the adjuster, review the scope, and — if the adjuster's scope misses damage we've documented — show our photos and measurements and provide a detailed repair estimate. Everyone bills insurance; nobody owns "you're our customer, not the insurance company's." This is the flagship position, so this hub gets first-class depth. Sub-pages for roof, water, and scope-review (educational denied/underpaid guidance + repair estimate). `FAQPage` block, CTA.
- **Required disclaimer (this hub + home + footer):** carry the standard block verbatim, near the insurance promise:
...
### Answer-engine layer (Learning Center: Q&A hub + guides + optional llms.txt)
...
- **llms.txt** (optional/experimental) at root: a plain-text map of the hubs, services, service area, NAP, and answer pages for AI systems that may consume it (no Google Search effect).
- **Review schema:** add `aggregateRating` / `Review` markup for structured-data / AI parsing, **only once real reviews exist on-page**, and never fabricate the count (see the [Competitor Analysis](COMPETITOR-ANALYSIS.md) guardrails). This does **not** produce organic star snippets — Google excludes self-serving LocalBusiness reviews from review rich results; stars show via the Google Business Profile / map pack.
...
| Scope review (denied/underpaid) | `Scope Review & Repair Estimate After a Denied or Underpaid Claim in Medina, OH` | `Scope Review & Repair Estimate in Medina County` |
...
- **Insurance-claim advocacy (the flagship cluster):** the roof, water, and scope-review pages each lead with the documented, plain-English "clear, documented, and honest at every step" process, not a sales pitch. The scope-review page explains, in educational terms, what a homeowner can do if a claim is denied or underpaid, and offers a professional repair estimate plus a referral to a licensed public adjuster or attorney for the claim dispute itself — it does not present denied- or underpaid-claim representation as a service Medwick performs. Storm-damage, the three claim pages, and emergency-water form the tight "whole storm" advocacy cluster, the differentiator, interlinked.
...
| insurance denied / underpaid / supplement my claim | `/insurance-claims/scope-review/` | **First: empty niche, educational scope-review + repair-estimate angle** |
...
| conversational / AI questions (who to call, does insurance cover, how fast) | `/learning-center/{question-slug}/` (+ optional `/llms.txt`) | **First: #1 gap, nobody is the cited answer** |
...
- Optionally publish an experimental `/llms.txt` at the root for AI systems that may consume it (no Google Search effect).
